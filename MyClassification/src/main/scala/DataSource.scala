...
class DataSource(val dsp: DataSourceParams)
  ...
  def readTraining(sc: SparkContext): TrainingData = {
    ...
    val labeledPoints: RDD[LabeledPoint] = PEventStore.aggregateProperties(
      ...
      required = Some(List("status", "market", "total_investment", "funding_rounds", "age_first_funding", "age_last_funding")))(sc)
      ...
      .map { case (entityId, properties) =>
        try {
          LabeledPoint(properties.get[Double]("status"),
            Vectors.dense(Array(
              properties.get[Double]("market"),
              properties.get[Double]("total_investment"),
              properties.get[Double]("funding_rounds"),
              properties.get[Double]("age_first_funding"),
              properties.get[Double]("age_last_funding")
            ))
          )
        } catch {
          case e: Exception => {
  ...
  : Seq[(TrainingData, EmptyEvaluationInfo, RDD[(Query, ActualResult)])] = {
    ...
    (0 until evalK).map { idx =>
      ...
      (
        ...
        testingPoints.map {
          p => (Query(p.features(0), p.features(1), p.features(2), p.features(3), p.features(4)), ActualResult(p.label))
        }
      )
    }
  }

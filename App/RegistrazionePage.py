...
class Registrazione(Frame):
    ...
    def create_widgets(self):
        ...
        self.password_textbox = Entry(self, width=20, textvariable=self.password_text)
        ...
        self.conferma_password_textbox = Entry(self, width=20, textvariable=self.conferma_password_text)
        self.conferma_password_textbox.place(relx=0.5, rely=0.6, anchor=CENTER)

        self.login_button = Button(self, text="Registrati", font=(18), width=10, command=lambda : self.registrato())
    ...
    def registrato(self):
        ...
        if(len(self.username_text.get()) < 3):
            ...
            error.place(relx=0.5, rely=0.65, anchor=CENTER)
            return
        if(len(self.password_text.get()) < 3):

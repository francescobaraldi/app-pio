...
class DataSource(val dsp: DataSourceParams)
  ...
  def getRatings(sc: SparkContext): RDD[Rating] = {
    ...
    val eventsRDD: RDD[Event] = PEventStore.find(
      ...
      eventNames = Some(List("interested", "notinterested")), // MODIFIED
      // targetEntityType is optional field of an event.
      targetEntityType = Some(Some("company")))(sc) // MODIFIED

    val ratingsRDD: RDD[Rating] = eventsRDD.map { event =>
      val rating = try {
        val ratingValue: Double = event.event match {
          // MODIFIED
          case "interested" => 4.0 // map a like event to a rating of 4.0
          case "notinterested" => 1.0  // map a like event to a rating of 1.0
